"""Forensic / anti-fraud heuristics over extracted transactions.

These are deterministic checks finance/audit teams actually use. They run
after the LLM stages and surface as additional `Anomaly` rows so the UI /
Excel report can flag them.

Rules implemented:
  * benford_first_digit_drift     - first-digit distribution of amounts
                                    drifts > 5% from Benford on > 50 samples.
                                    A classic embezzlement signal.
  * vendor_concentration_spike    - one vendor accounts for > 35% of total
                                    outflow in the period.
  * velocity_burst                - > N transactions in a single day,
                                    where N is 3x the median daily count.
  * weekend_or_holiday_activity   - business account showing weekend or
                                    common-US-holiday transactions.
  * round_number_excess           - > 20% of withdrawal amounts end in
                                    .00 with magnitude >= $1000 (rounded
                                    payments are statistically suspicious
                                    on B2B activity).

Each rule's threshold is conservative -- false positives are preferable to
silent passes in a financial system. Calibrate per tenant in production.
"""
...
from extractor.schemas import Anomaly, Transaction


# Benford's expected first-digit distribution.
_BENFORD = {d: math.log10(1 + 1 / d) for d in range(1, 10)}

# Federal US bank holidays in 2024-2025 (sufficient for the sample; in
# production we'd compute these per year / region).
_HOLIDAYS = {
...
def forensic_anomalies(transactions: list[Transaction]) -> list[Anomaly]:
    """Run all rules; return the union of findings.

    Anomalies are emitted with `kind="size_outlier"` repurposed for finance
    rules (the schema is closed). For richer kinds you would extend the
    `Anomaly.kind` Literal. Each message starts with `[<rule>]` so the
    Excel report and UI can group them.
    """
    out: list[Anomaly] = []
    if not transactions:
        return out
