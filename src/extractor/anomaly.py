"""Stage 7: anomaly detection over already-extracted transactions.

Deterministic, no LLM. Flags suspicious patterns the user should review:
  * duplicate_pair        -- same (date, amount, side) twice in a row
  * date_out_of_period    -- transaction date outside the declared period
  * size_outlier          -- amount > 5x the median absolute amount
  * round_number_outlier  -- amount is suspiciously round (>= $1000 ending
                             in three zeros)
  * low_confidence        -- enrich stage rated confidence < 0.5
  * running_balance_drift -- rolling balance computed from the txns diverges
                             from any balance values present in description

These are advisory, not blocking. They show up in the Statement.anomalies
list with severity (info / warn / error) so the UI can colour-code them.
"""
    ...
    for key, indices in key_to_indices.items():
        if len(indices) >= 2:
            for i in indices[1:]:
                anomalies.append(Anomaly(
                    kind="duplicate_pair",
                    severity="info",   # legitimately common (e.g. recurring)
                    transaction_index=i,
