"""LLM prompts used by extraction stages.

Each prompt is split into a SYSTEM block (cached) and an INSTRUCTION block.
The system block contains rules + few-shot guidance that does not change per
statement, so it is sent with cache_control to benefit from Anthropic prompt
caching across the many parallel calls in a 99-page document.
"""
