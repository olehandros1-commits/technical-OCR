...
class ForensicAnomalyDetector:
    ...
    def _first_digit(self, x: float) -> int | None:
        x = abs(x)
        if x <= 0:
            return None
        while x < 1:
    ...
    async def detect(self, transactions: list[Transaction]) -> list[Anomaly]:
        out: list[Anomaly] = []
        if not transactions:
            return out
        ...
        digits = [d for d in (self._first_digit(a) for a in amounts) if d is not None]
        if len(digits) >= 50:
        ...
        daily_indices: defaultdict[date, list[int]] = defaultdict(list)
        for i, t in enumerate(transactions):
            td = self._parse_date(t.date)
            if td:
                daily_counts[td] += 1
                daily_indices[td].append(i)
        if daily_counts:
                ...
                if n > thr:
                    out.append(
                        Anomaly(
                            kind="size_outlier",
                            severity="info",
                            transaction_index=daily_indices[day][0],
                            message=(
                                f"[velocity] {day.isoformat()} has {n} transactions "
                ...
                out.append(
                    Anomaly(
                        ...
                        message=(
                            ...
                            f"{td2.strftime('%A %Y-%m-%d')}. Most business accounts "
                            "don't post on weekends; worth a glance."
                        ),
                    )
                )
            elif td2 in _HOLIDAYS:
                out.append(
                    Anomaly(
                        kind="size_outlier",
                        severity="info",
                        ...
                        message=(
                            ...
                            f"({td2.isoformat()}). Verify the date is correct."
                        ),
                    )
                )

        big_withdrawals = [
            t.withdrawal for t in transactions if t.withdrawal is not None and t.withdrawal >= 1000
        ]
        if len(big_withdrawals) >= 20:
            round_ones = sum(
                1 for w in big_withdrawals if abs(w - round(w)) < 0.01 and int(w) % 100 == 0
            )
