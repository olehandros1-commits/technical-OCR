...
class DemoReplayPlayer:
    ...
    async def replay(
        ...
        *,
        ...
        for r in results:
            label = (
                f"{r['account']['period']['start']} acct "
                f"{r['account'].get('account_last4') or '?'}"
            )
            await emit("segment_start", {
                "label": label, "chars": 20000, "backend": "demo-replay",
            }, 0.05)
            await emit("summary_done", {
                "label": label,
                "bank": r["account"].get("bank"),
                "last4": r["account"].get("account_last4"),
                "period": (
                    f"{r['account']['period']['start']} -> "
                    f"{r['account']['period']['end']}"
                ),
                "elapsed_s": 0.4,
            }, 0.2)
            await emit("transactions_done", {
                "label": label,
            ...
            if anomalies:
                ...
                await emit("anomalies_found", {
                    "label": label, "count": len(anomalies), "by_kind": by_kind,
                }, 0.0)
            await emit("segment_done", {
