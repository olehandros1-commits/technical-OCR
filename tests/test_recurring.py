...
async def test_monthly_subscription_detected():
    txns = [
    ...
    groups = await detect_recurring(txns, min_occurrences=3)
    assert len(groups) == 1
    g = groups[0]
...
async def test_one_off_transactions_not_grouped():
    txns = [
        _tx("2025-01-15", "ACME", 100, "withdrawal"),
        _tx("2025-02-01", "OTHER", 50, "withdrawal"),
    ]
    assert await detect_recurring(txns) == []
...
async def test_high_variance_cluster_dropped():
