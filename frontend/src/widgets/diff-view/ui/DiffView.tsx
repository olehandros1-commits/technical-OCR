...
import { diffExtractions } from "@/features/diff-extractions";
import type { ExtractionDiff } from "@/features/diff-extractions";
...
function DiffRowList({ rows }: { rows: unknown[] }) {
  return (
    <table className="diff-table">
        ...
        {rows.slice(0, 20).map((r, i) => {
          ...
          return (
            <tr key={i}>
              ...
              <td className="num">{row.deposit != null ? `$${row.deposit}` : ""}</td>
              <td className="num">{row.withdrawal != null ? `$${row.withdrawal}` : ""}</td>
            </tr>
          );
        })}
      </tbody>
